...
function roomStatusText() {
  ...
  if (state.roomRole === "player2") return `Joined room ${state.roomCode} as Player 2. ${currentPlayerName()}'s turn.`;
  if (state.roomRole === "player1") return `Room ${state.roomCode} is hosted. Share the code with Player 2. ${currentPlayerName()}'s turn.`;
...
pvpEls.hostRoom.addEventListener("click", (event) => {
  ...
  state.roomJoined = true;
  saveRoomState();
  setMessage("Room hosted", `Room code ${state.roomCode} is ready. Share it with Player 2.`);
  render();
}, true);
