...
// A circular pie-chart "wheel" — colored wedges divided EQUALLY (not to
// true probability scale — a 0.1%-true-scale Mythical sliver would be
...
function buildWheelOverlay(isSeasonal) {
  ...
  // Back to the generated SVG wheel (the "old" wheel/animation), sized
  // to exactly cover the matching wheel baked into the background image
  // — same position/size overlay, just the wheel itself reverted.
  const wheelEl = el('div', { class: 'ttd-gacha-wheel' + (isSeasonal ? '' : ' ttd-gacha-wheel-normal') });
  wheelEl.innerHTML = buildOddsWheel(isSeasonal ? SEASONAL_WHEEL_SLICES : NORMAL_WHEEL_SLICES, WHEEL_SIZE);
